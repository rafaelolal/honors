...
static double max_bandwidth() {
  ...
  // CUDA 13 removed memoryClockRate from cudaDeviceProp; query it directly
  int memory_clock_rate = 0;
  cudaDeviceGetAttribute(&memory_clock_rate, cudaDevAttrMemoryClockRate, 0);
  const std::size_t mem_freq =
      static_cast<std::size_t>(memory_clock_rate) * 1000; // kHz -> Hz
...
void simulate(int height, int width, const ContainerT &in, ContainerT &out) {
  cuda::std::mdspan<const float, cuda::std::dextents<int, 2>> temp_in(thrust::raw_pointer_cast(in.data()), height, width);
